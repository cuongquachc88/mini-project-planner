import type { DbWorkItem, Priority, WorkItemType } from '@/types/db'
  ...
  const r = await db.query<DbWorkItem>(
    `INSERT INTO work_items
      (id, project_id, title, type, priority, stage_id, sprint_id, epic_id, milestone_id,
       assignee_id, reporter_id, story_points, due_date, description, position)
    ...
    [
      ...
      data.dueDate ?? null, data.description ?? null, pos,
    ],
  )
  return r.rows[0]
}
