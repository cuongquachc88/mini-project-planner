...
export async function updateWorkItem(id: string, data: Partial<DbWorkItem>): Promise<void> {
  ...
  const allowed: (keyof DbWorkItem)[] = [
    ...
    'position', 'backlog_pos', 'parent_id', 'acceptance_criteria', 'tech_notes', 'wiki_page_id',
  ]
  const sets: string[] = []
  const vals: unknown[] = []
  let i = 1
...
export async function getAttachments(workItemId: string): Promise<DbWorkItemAttachment[]> {
  const db = getDb()
  const r = await db.query<DbWorkItemAttachment>(
...
export async function addAttachment(workItemId: string, filename: string, mimeType: string, data: string): Promise<DbWorkItemAttachment> {
  const db = getDb()
  const id = crypto.randomUUID()
...
export async function getAttachmentData(id: string): Promise<{ data: string; mime_type: string; filename: string } | null> {
  const db = getDb()
  const r = await db.query<{ data: string; mime_type: string; filename: string }>(
    `SELECT data, mime_type, filename FROM work_item_attachments WHERE id = $1`,
    [id],
  )
  return r.rows[0] ?? null
}
